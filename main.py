...
st.title("4G Cell File Generator")
...
if st.button("Generate 4G Cell TXT"):
    ...
    row2 = f"{system}\t{site}\t{lat}\t{lon}\t{azi2}\t{cell_name}\t{cell_id}\t{pci}\t{tac}\t{arfcn}\t{mcc}\t{mnc}\t{ant_bw}"
    row3 = f"{system}\t{site}\t{lat}\t{lon}\t{azi3}\t{cell_name}\t{cell_id}\t{pci}\t{tac}\t{arfcn}\t{mcc}\t{mnc}\t{ant_bw}"

    output_text = header + "\n" + row1 + "\n" + row2 + "\n" + row3

    st.download_button(
        label="Download 4G_cell_output.txt",
        data=output_text,
        file_name="4G_cell_output.txt",
        mime="text/plain"
    )
